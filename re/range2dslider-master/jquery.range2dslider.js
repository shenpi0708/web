if (location.hostname == "") {
  ...
  var ros = new ROSLIB.Ros({
	url: "wss://" + location.hostname + ":9090",
  });
		 ...
		 var drag = false,
			...
			xdSoftDraggableDrag = function( event ){
				if( drag&&!options.disabled ){
					...
					getX = oldLeft+(options.x=='right'?-1:1)*(out.x-oldX);
					getY = oldTop+(options.y=='bottom'?-1:1)*(out.y-oldY);
					angle=Math.abs(Math.atan((getY-limitY/2)/(getX-limitX/2)) )
					...
					if( options.allowAxisMove=='both' || options.allowAxisMove=='y'){
						...
						if(  length >limitY/2 ) {
							if(getY>limitY/2)
								newY = (limitY/2)*Math.sin(angle)+limitY/2;
							else
								newY = -limitY/2*Math.sin(angle)+limitY/2;							
						}
